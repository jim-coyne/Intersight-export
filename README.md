...
Each policy type is exported two ways:
- **Flat array file**: `exports/bios_policies.json` (JSON array of all BIOS policies)
- **Individual files**: `exports/bios_policies/<PolicyName>.json` (one file per policy)

### Step 2 — Choose a policy to copy

List available exported BIOS policies:

```bash
ls exports/bios_policies/
```

Example output:
```
SMF.json   default-bios-policy.json   demo-bios-policy.json
```

### Step 3 — Prepare the push folder

The push playbook reads `push/bios_policies.json` as a JSON **array**. Wrap the single policy file in an array:
...
To push **multiple** policies, add each object to the array instead.

### Step 4 — Push the policy to Intersight

Use `name_suffix` to create the new policy with a different name (avoids conflicts with the source policy).
...
- `changed=1` — the policy was **created** (POST) for the first time
- `changed=0` — the policy **already existed** and was patched with no differences (idempotent)
- `failed=1` — see Troubleshooting below

The new policy will appear in Intersight as `SMF-NEW`.

### Step 5 — Verify (optional)

Re-run the pull playbook and check the exports:

```bash
ansible-playbook pull_intersight_config.yml
ls exports/bios_policies/
# Should now include SMF-NEW.json
```
...
| Parameter | Default | Description |
|---|---|---|
| `target_org` | `"default"` | Intersight organization name for the new policies |
| `push_types` | `""` (all) | Comma-separated list of policy types to push. Empty = push all. |
| `import_dir` | `./exports` | Directory containing the JSON files to push |
| `name_prefix` | `""` | Prefix added to every object name on creation |
| `name_suffix` | `""` | Suffix added to every object name on creation |

### Examples

Push only BIOS policies with a suffix:
```bash
ansible-playbook push_intersight_config.yml \
  -e "push_types=bios_policies" \
  -e "name_suffix=-COPY"
...
Push BIOS and NTP policies to a different org:
```bash
ansible-playbook push_intersight_config.yml \
  -e "target_org=my-org" \
  -e "push_types=bios_policies,ntp_policies" \
...
Push all policy types from the exports directory:
```bash
ansible-playbook push_intersight_config.yml \
  -e "import_dir=$(pwd)/exports" \
  -e "name_suffix=-COPY"
...
`push_types` accepts a comma-separated list of any of the following:

| Value | Object Type |
|---|---|
| `pools_ip` | IP Pools |
| `pools_mac` | MAC Pools |
| `pools_uuid` | UUID Pools |
| `pools_wwn` | WWN (Fibre Channel) Pools |
| `eth_adapter_policies` | Ethernet Adapter Policies |
| `eth_network_policies` | Ethernet Network Policies |
| `eth_qos_policies` | Ethernet QoS Policies |
| `fc_adapter_policies` | FC Adapter Policies |
| `fc_network_policies` | FC Network Policies |
| `fc_qos_policies` | FC QoS Policies |
| `lan_policies` | LAN Connectivity Policies |
| `vnic_eth_ifs` | vNIC Ethernet Interfaces |
| `san_policies` | SAN Connectivity Policies |
| `vhba_fc_ifs` | vHBA Fibre Channel Interfaces |
| `adapter_config_policies` | Adapter Config Policies |
| `bios_policies` | BIOS Policies |
| `boot_policies` | Boot Order Policies |
| `firmware_policies` | Firmware Policies |
| `imc_access_policies` | IMC Access Policies |
| `kvm_policies` | KVM Policies |
...
| `server_profile_templates` | Server Profile Templates |
| `server_profiles` | Server Profiles |
| `chassis_profiles` | Chassis Profiles |
| `domain_profiles` | Domain Profiles |
...
### `'target_org_moid' is undefined`
The org lookup task ran but couldn't find the organization. Verify `target_org` matches the exact organization name in Intersight (case-sensitive).

### `Policy type is already attached to profiles`
The source policy is linked to server profiles. The push playbook strips the `Profiles` field automatically, but if the error appears it means a previous push created the policy with profile references still attached. Delete the partially-created policy from Intersight and re-run.

### `Relationship to organization is missing`
The Organization reference in the API body is invalid. Ensure `target_org` exactly matches the organization name in Intersight and that your API key has access to it.

### `Cannot modify read-only relationship Organization`
A PATCH is attempting to change the Organization field. This happens when the `query_params` filter finds the wrong existing policy. Verify the policy name in `push/bios_policies.json` combined with `name_prefix`/`name_suffix` produces a unique name.

### `changed=0` when expecting a new policy
The policy already exists in Intersight with the exact same name. Either delete it first or use a different `name_suffix`.
